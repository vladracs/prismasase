...
def push_policy_security(cgx_session, loaded_config):
    ...
    for ngfwsetname in ngfwsetconfig_yaml.keys():
        ...
        if ngfwsetname in ngfwpolicyset_name_config.keys():
            ...
            # Extract rules and immediately remove from set_yaml to avoid PUT errors later
            ...
            # ... (Rule Delete Logic remains same) ...
            for rulename in rules_ctrl.keys():
                if rulename not in rules_yaml.keys():
                    data = rules_ctrl[rulename]
            ...
            ############################################################################
            # Security Set - Update (FIXED FOR DEFAULT SETS AND COUNT MISMATCH)
            ############################################################################
            set_data_yaml = translate_set(setdata=set_yaml, setid=set_ctrl["id"], set_type=SECURITY, action=N2ID)
            
            # PROTECTION: Security API rejects the rules list inside a SET update
            if "ngfwsecuritypolicyrules" in set_data_yaml:
                del set_data_yaml["ngfwsecuritypolicyrules"]

            # PROTECTION: Default Security Sets do not allow order changes
            ...
            if len(confdelta) > 0:
                ...
                if is_default_set:
                    # Strip order from default sets to avoid 400 error
                    if "policyrule_order" in data:
                        del data["policyrule_order"]
                        print("\tINFO: Stripping order from Security Default Set: {}".format(ngfwsetname))

                # Only attempt PUT if there is still data to change
                if len(compareconf(data, set_ctrl)) > 0:
                    ...
                    elif "POLICYRULE_COUNT_MISMATCH" in str(resp.cgx_content) and not is_default_set:
                        # FALLBACK: Try order-only update for custom sets
                        print("\tRetrying custom set update (Order-only)...")
                        minimal = {"id": data["id"], "name": data["name"], "policyrule_order": data.get("policyrule_order", [])}
                        resp = cgx_session.put.ngfwsecuritypolicysets(ngfwsecuritypolicyset_id=data["id"], data=minimal)
                        if resp.cgx_status:
                            print("Updated Security Set (Order Fixed): {}".format(ngfwsetname))
                    else:
            ...
            ############################################################################
            # Security Set - New Create (POST handles everything fine)
            ############################################################################
            rules_yaml = extractfromyaml(loaded_config=set_yaml, config_type=SECURITY_POLICY_RULES)
            if SECURITY_POLICY_RULES in set_yaml.keys():
                del set_yaml[SECURITY_POLICY_RULES]
            ...
            if resp.cgx_status:
                ...
                for rulename in rules_yaml.keys():
                    rule_yaml = rules_yaml[rulename]
                    rule_data_yaml = translate_rule(rule=rule_yaml, action=N2ID, rule_type=SECURITY)
                    resp = cgx_session.post.ngfwsecuritypolicyrules(ngfwsecuritypolicyset_id=set_id, data=rule_data_yaml)
                    if resp.cgx_status:
                        print("\tCreated Rule: {}".format(rulename))
                        ngfwrule_name_id[rulename] = resp.cgx_content.get("id", None)

                # Finalize Order for New Set
                if policyrule_order:
    ...
    # ... (Keep original Stacks logic below) ...
    # Process Stacks
...
def push_policy_performance(cgx_session, loaded_config):
    """
    HEAVY REFACTORED: Restores Performance Policies using DictDiffer and ETag management.
    Handles auto-generated system rules to prevent 400 errors.
    """
    print("INFO: Reviewing Performance Configuration for updates")
    
    # 1. Get Live Data and build lookup for Etags
    perf_sets_resp = cgx_session.get.perfmgmtpolicysets()
    if not perf_sets_resp.cgx_status:
        print("ERR: Could not retrieve live Performance Sets")
        return

    perf_sets_ctrl = {i["name"]: i for i in perf_sets_resp.cgx_content.get("items", [])}
    perf_sets_yaml = loaded_config.get("performance_sets", [])

    for entry in perf_sets_yaml:
        set_name = list(entry.keys())[0]
        set_yaml = entry[set_name]
        rules_yaml = set_yaml.pop("rules", [])
        set_yaml["name"] = set_name

        # --- PART A: SYNC THE POLICY SET ---
        if set_name in perf_sets_ctrl:
            set_ctrl = perf_sets_ctrl[set_name]
            # Use Heavy Compare logic
            if len(compareconf(set_yaml, set_ctrl)) > 0:
                data = update_payload(set_yaml, set_ctrl)
                resp = cgx_session.put.perfmgmtpolicysets(perfmgmtpolicyset_id=data["id"], data=data)
                if resp.cgx_status: print(f"Updated Performance Set: {set_name}")
            else:
                print(f"No Changes to Performance Set: {set_name}")
            set_id = set_ctrl["id"]
        else:
            # New Create
            resp = cgx_session.post.perfmgmtpolicysets(data=set_yaml)
            if resp.cgx_status:
                print(f"Created Performance Set: {set_name}")
                set_id = resp.cgx_content.get("id")
            else:
                print(f"ERR: Could not create Perf Set {set_name}: {resp.cgx_content}")
                continue

        # --- PART B: SYNC THE RULES ---
        # Fetch current rules for this set to get IDs and Etags
        rules_resp = cgx_session.get.perfmgmtpolicysets_perfmgmtpolicyrules(perfmgmtpolicyset_id=set_id)
        rules_ctrl = {r["name"]: r for r in rules_resp.cgx_content.get("items", [])}

        for r_entry in rules_yaml:
            r_name = list(r_entry.keys())[0]
            r_data = r_entry[r_name]
            r_data["name"] = r_name

            # Translate SLA Threshold Names to IDs
            tp_name = r_data.get("thresholdprofile_id")
            if tp_name in perf_threshold_name_id:
                r_data["thresholdprofile_id"] = perf_threshold_name_id[tp_name]

            if r_name in rules_ctrl:
                # RULE EXISTS: Update via PUT
                r_ctrl = rules_ctrl[r_name]
                if len(compareconf(r_data, r_ctrl)) > 0:
                    r_payload = update_payload(r_data, r_ctrl)
                    cgx_session.put.perfmgmtpolicysets_perfmgmtpolicyrules(
                        perfmgmtpolicyset_id=set_id, 
                        perfmgmtpolicyrule_id=r_payload["id"], 
                        data=r_payload
                    )
                    print(f"\tUpdated Rule: {r_name}")
                else:
                    print(f"\tNo Changes to Rule: {r_name}")
            else:
                # RULE MISSING: Handle System Rules vs Custom Rules
                system_names = ["Default Performance Policy Rule for All Apps", "Default Performance Policy Rule for All Media Apps"]
                if r_name in system_names:
                    # Logic: System rules might have been auto-created with a different name by the tenant.
                    # Usually Performance doesn't auto-create 'default' like Path does, 
                    # but we use POST here for custom and catch errors.
                    pass

                resp = cgx_session.post.perfmgmtpolicysets_perfmgmtpolicyrules(perfmgmtpolicyset_id=set_id, data=r_data)
                if resp.cgx_status:
                    print(f"\tCreated Rule: {r_name}")
                else:
                    print(f"\tERR: Could not push Rule {r_name}: {resp.cgx_content}")
